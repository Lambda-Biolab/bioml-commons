...
try:
    from bioml_private_retrieval import get_pdf_via_provider_a, get_pdf_via_provider_b
except ImportError:
    get_pdf_via_provider_a = None
    get_pdf_via_provider_b = None
...
The private package exposes provider-anonymized aliases (`get_pdf_via_provider_a`, `get_pdf_via_provider_b`) specifically so public consumers can reference the API without revealing provider identities. Functions guard with `is None` checks; harness scripts guard with `.exists()` checks. When the private package or data is absent, the relevant code paths gracefully degrade — they do not error.
...
1. **No private filenames in public repos.** Each public agent repo runs a `scripts/check_private_assets.sh` first-step in CI that fails the build if a private asset appears in the public tree. The check uses an allowlist for the `retrieval/` directory (only the public OA modules are permitted; new files require an ADR-overriding the policy) plus a literal block list for the canonical Tier-A evaldata filenames. Override via `ALLOW_PRIVATE_ASSETS=1` with PR-description justification.
...
2. **Make the entire `deep-research-agent` private.** Rejected: gives up community signal value for ~2,000 lines of public-good infrastructure (OA retrieval tiers 1-4, AGENTS.md, architecture docs, scoring harness) to protect a smaller subset of legally-exposed code and curated evaldata. Wrong cost/benefit.
...
- Wrap the `bioml-private-retrieval` import in `try/except ImportError` with `is None` guards at call sites; reference only the provider-anonymized aliases (`get_pdf_via_provider_a`, `get_pdf_via_provider_b`), never source-named symbols.
